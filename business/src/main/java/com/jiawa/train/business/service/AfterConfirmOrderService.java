...
public class AfterConfirmOrderService {
    ...
    public void afterDoConfirm(DailyTrainTicket dailyTrainTicket, List<DailyTrainSeat> finalSeatList, List<ConfirmOrderTicketReq> tickets, ConfirmOrder confirmOrder){
        for (int j = 0; j < finalSeatList.size(); j++) {
            ...
            //Integer endIndex = 7;
            //Integer minStartIndex = startIndex之前碰到的最后一个零;
            //Integer maxStartIndex = endIndex - 1;
            //Integer minEndIndex = startIndex + 1;
            //Integer maxEndIndex = endIndex之后碰到的最后一个零;
            Integer startIndex = dailyTrainTicket.getStartIndex();
            Integer endIndex = dailyTrainTicket.getEndIndex();
            char[] chars = seatForUpdate.getSell().toCharArray();
            System.out.println("座位"+dailyTrainSeat.getCarriageSeatIndex()+"的sell情况："+seatForUpdate.getSell());
            Integer maxStartIndex = endIndex - 1;
            Integer minEndIndex = startIndex + 1;
            ...
            for (int i = endIndex; i < maxEndIndex; i++) {
                if(chars[i] == '1'){
                    maxEndIndex = i;
                    break;
                }
            ...
            memberTicketReq.setDate(dailyTrainSeat.getDate());
